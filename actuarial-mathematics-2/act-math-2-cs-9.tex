...
\[
 \hat{G}(t_j) = \frac{S(t_j) - \text{RZ}(t_j) - \text{SO}(t_j)}{\sum_{x} L_x(t_j) \cdot k_x^{\text{eff}}(t_j)}
\]
for \( j = 0, -1, -2 \).
3. Choose calculatory head claims \( G^{\text{calc}}(t_2) \) by extrapolation of value from step 2.
4. Choose calculatory head claims via Rusam-ansatz by \( K_x^{\text{calc}}(t_2) = G^{\text{calc}}(t_2) k^{\text{calc}}_x \)
