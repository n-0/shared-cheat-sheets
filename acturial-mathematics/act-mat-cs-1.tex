...
Annuity-Immediate (in arrears) \\
(PV one period before first payment, FV at time of last payment) aka payments are made at the end of a period, \\
...
(PV at time of first payment, FV one period after last payment) aka payments are made at the start of a period,\\
$PV = \ax**{\angl n} = \sum_{j=0}^{n-1} v^j = \frac{1-v^n}{d} = (1 + i)\ax{\angl n}$
$ = (\frac{i}{d})\ax{\angl n} = 1 + a_{\angl{n -1}}$ \\
$ FV = \sx**{\angl n} = \sum_{j=1}^{n} (1 + i)^j = \frac{(1 + i)^n - 1}{d} = (1+i)\sx{\angl n} = (\frac{i}{d})\sx{\angln} = \sx{\angl{n+1}} - 1 $ \\
\\
...
in arrears \\
$\ax[m|]{\angl n} = \sum_{t=m+1}^{n + m} v^t = v^m \frac{1 - v^n}{i} = \ax**[m+1|]{\angl n} = v^m \ax{\angl n} = \ax{\angl{m+n}} - a_{\angl{m}}$ \\
in advance \\
$ \ax**[m|]{\angl n} = \sum_{t=m}^{n + m -1} v^t = v^m\frac{1 - v^n}{1 - v}$ \\
\\
Continuous analogs \\
$ PV = \int_0^n f(t)e^{-\int_{0}^{t} \delta_\tau d\tau} dt = \int_{0}^n f(t)e^{-\delta t} dt $  \\
$ FV = \int_0^n f(t)e^{-\int_{t}^{n} \delta_\tau d\tau} dt = \int_{0}^n f(t)e^{-\delta (n-t)} dt $ \\
where the second eq. holds if the force of interest is constant.
...
A more involved result, which sometimes allows the explicit derivation is
\( \psi_{t+1}(c_0) = 1 - F(c_0) + \int_{-\infty}^{c_0} \psi_t(c_0 - y)dF(y) \)
\( \psi(c_0) = 1 - F(c_0) + \int_{-\infty}^{c_0} \psi(c_0 - y)dF(y) \)
where \( F \) is the distribution of \( S_1 - \pi_1 \).
