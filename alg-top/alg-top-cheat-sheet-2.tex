...
On a more abstract note, the theorem asserts that the homology \( H_n \) is a functor between the homotopy category \( \textbf{HTop} \) 
...
Define affine (singular) simplices, the cone operator and the barycentric subdivision operator
with std. facts.
...
he open ball around it, is contained as in \( B_\lambda(p) \subset U_i \) for some \( i \). Thus there is some \( m \) such that
