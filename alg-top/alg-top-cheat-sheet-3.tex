...
Proof:
By the definition of small chains, the image of each singular simplex is either contained in \( U \) or \( V \), making \( g \) surjective.
...
More so there is a unique \( a \in C_{n-1}(U \cap V) \) with \( f(a) = \partial(c_U, c_V) \), but then \( a = \partial c_U = -\partial c_V  \)
...
Note that the reduced relative homology agrees completely with the unreduced version, since it creates the sequence:
\( \dots \tilde{H}_n(A) \longrightarrow \tilde{H}_n(X) \longrightarrow H_n(X, A) \longrightarrow \tilde{H}_{n-1}(A) \longrightarrow \dots \),
Moreover \( \tilde{H}_n(X) \simeq H_n(X, \{p\}) \).
