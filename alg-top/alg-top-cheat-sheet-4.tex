...
and note that \( \Phi(e^n) = \Phi(e^n) = [0, 1) \times [0, 2\pi]\),
...
\(\{\varphi_{\alpha}^{n-1} : S^{n-1} \to X^{n-1} \mid \alpha \in A_n\}\) so that
\[
...
The second part then follows directly for \( \text{dim }X < \infty \) the special case of infinity is more intricate. 
