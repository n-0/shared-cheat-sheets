...
let 
\(
d_n^i : \operatorname{Hom}_R(P_{n}, D) \to \operatorname{Hom}_R(P_{n+1}, D)
\)
for all \(n \geq 1\). Define
\(
\operatorname{Ext}_R^n(A, D) = \ker d_{n}^i / \operatorname{im} d_{n-1}^i,
\)
...
For finitely generated modules/groups, \( A \) we have the luxury of a presentation \( 0 \longrightarrow F_1 \xrightarrow{\iota} F_0 \xrightarrow{\pi} A \longrightarrow 0 \),
thus after removing \( A \) we get the not necessarily exact sequence
%and exactness preserved for the \( \text{Hom}(-, D) \) functor

\( 0 \longrightarrow \text{Hom}(F_0, D) \xrightarrow{\iota^\vee} \text{Hom}(F_1, D) \longrightarrow 0 \)

Hence, \( \operatorname{Ext}(A, D) = \text{Hom}(F_1, D) / \text{im}(\iota^\vee) \).
...
\(
\operatorname{Tor}_n^R(D, B) = \ker(1 \otimes d_n) / \operatorname{im}(1 \otimes d_{n+1})
\)

where \( \operatorname{Tor}_0^R(D, B) = (D \otimes P_0) / \operatorname{im}(1 \otimes d_1) \). 
The group \( \operatorname{Tor}_n^R(D, B) \) is called the \( n \)-th homology group derived from the covariant (in both arguments actually) functor \(D \otimes - \). 
...
Then \(  \operatorname{Tor}(A, D) = H_1(D \otimes F_\bullet) = \text{id}_D \otimes \text{ker }\iota \)
...
4. \(\text{Tor}(A, G) \simeq \text{Tor}(G, A)\)
...
\(0 \longrightarrow H_n(X) \otimes G \longrightarrow H_n(X;G) \longrightarrow \text{Tor}(H_{n+1}(X), G) \to 0.\)
...
3.
There are multiple equivalent definitions of flat modules \( M \),
...
\(M_{\text{tor}}\) is the torsion submodule of M. In particular \( v \otimes m = 0 \) if and only if \( v = 0 \) or \( m \in M_{\text{tor}} \)
