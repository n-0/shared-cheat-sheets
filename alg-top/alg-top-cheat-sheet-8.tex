...
\( (\varphi \cup \psi) \cup \mu = \varphi \cup (\psi \cup \mu) \)
...
is an identity element for \( \cup \), that is \( 1_{C^0} \cup \varphi = \varphi \cup 1_{C^0} = \varphi \).
...
2. If \(R\) has a unit 1, then \(f^*(1_{C_0}) = 1_{C_0} \in C^0(X; R)\).
...
also called cup-product, satisfying the same properties as the regular one.
...
Since the homology groups are all free, the universal coefficient theorem for cohomology gives \(H^k(T; \mathbb{Z}) \cong \operatorname{Hom}(H_k(T), \mathbb{Z}), \forall k \in \mathbb{Z}\). 
Thus
\(
...
For \(n \geq m \geq 0\), define the cap-product map \(\cap : C^m(X; R) \times C_n(X; R) \to C_{n-m}(X; R)\) as follows:

1. For \(\varphi \in C^m(X; R)\), \(\sigma : \Delta^n = ([e_0, \dotsc, e_n]) \to X\) and \(r \in R\), set 

\(\varphi \cap (r\sigma) := \varphi(\sigma|_{[e_{n-m}, \dotsc, e_n]}) \cdot r \sigma|_{[e_0, \dotsc, e_{n-m}]} \in C_{n-m}(X; R).\)
...
and
\( \partial(\varphi \cap c) = (-1)^m(\varphi \cap (\partial c) -(\delta \varphi) \cap c ) \in C_{n-m-1}(X;R) \).
...
\( H_k(M, M \setminus \{p\}; R) \simeq H_k(U, U \setminus \{p\};R) \simeq \tilde{H}_{k-1}(U \setminus \{p\};R) \simeq \tilde{H}_{k-1}(S^{n-1};R) \)
\( \simeq R \) if \( k = n \) and \( 0 \) otherwise.

A local orientation at \( p \) of \( M \)  is an element \( o_p \in H_n(M, M \setminus \{p\};R) \), s.t. \( H_n(M, M \setminus \{p\};R) = Ro_p \)  a generator of the 
...
The proof is quite involved and consists of the subsequent steps, first assume that the statement holds for a family of compact sets and show it still holds for the union of them,
then prove it in general for convex compact sets \( K \), then
...
to see that \( H_n(M, M \setminus \{p\};R) \simeq H_n(M, M \setminus \{p\}) \otimes R \), thus an orientation is given by \( o_p \otimes r \) for some \( R \),
however if the manifold is not orientable, a coherent manifold must satisfy along a path which would reverse orientation \( o_p \otimes r = - o_p \times r \), yielding the statement.
...
\(
D_M := (-\cap o_M) \colon H^k(M; R) \xrightarrow{\cong} H_{n-k}(M; R)
\)
...
\( \chi_k : S^{k-1} \xrightarrow{\phi_k} \mathbb{P}_{k-1}(\mathbb{R}) \xrightarrow{q_k} \mathbb{P}_{k-1}(\mathbb{R})/(\mathbb{P}_{k-1}(\mathbb{R}) \setminus e_{k-2}) \simeq S^{k-1}\),
...
w.l.o.g. on one component \( \chi_k \) acts like a homeomorphism while on the other antipodal, thus  \( \text{deg}(\chi_k) = 1 + (-1)^k \) which is \( 2 \) if 
...
for cell complexes \( H_p(X^{(n)}) \simeq H_p(X^{(p)}) \) for \( n > p + 1\), b
...
Since \(H_{p+1}(\mathbb{P}_n(\mathbb{C})) = 0 = H_{p}(\mathbb{P}_n(\mathbb{C})) \) by induction it follows that
