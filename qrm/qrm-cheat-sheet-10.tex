...
\( \{A_1 \geq L_{1}\} \) is an important event, but if everything goes down the drain,
one might be interested in the size of recovery in the event of insolvency.

The solvency condition \( \text{Rec}V@R_{\gamma}(\delta E_1, L_1) \leq E_0 \)
is equivalent to requiring that for all recovery fractions λ ∈ [0, 1] the recovery probabilities satisfy
\( \mathbb{P}[A_1 < \lambda L_1] \leq \gamma(\lambda) \iff \mathbb{P}[A_1 \geq \lambda L_1] \geq 1 - \gamma(\lambda) \).

NOT FOUND
 however insolvency is still possible (after all the V@R is not taken at \( \alpha = 1 \).
