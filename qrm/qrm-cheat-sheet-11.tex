What is the square root formula in the standard formula of Solvency II (for firms
that do not use an internal model)?
...
Demonstrate that this aggregation works for jointly normal risk and Mean-V@R
(and also Mean-AV@R). Why is the formula still problematic?
 
The square root formula resembles the structure of the formula of the variance \(\text{Var}\) or standard deviation \(\sigma = \sqrt{\text{Var}}\), 
respectively, of two random variables with linear correlation \(\rho\):
\(
...
How do internal models compute risk measures of aggregate positions?
...
Explain capital allocation, risk contributions, and the properties complete capital
allocation and diversification. Give examples of such principles.
...
Show that the Euler allocation satisfies for coherent risk measures complete capital
allocation and diversification.

Let \( f : \mathbb{R}^n \ {0} \to \mathbb{R} \) be differentiable and positive homogeneous of order \( \alpha > 0 \) i.e.,
\( f(t \mathbf{w} = t^\alpha f(\mathbf(w) \) for all \( t > 0, \mathbf{w} \in \mathbb{R}^n \setminus \{0\} \)
Define the weighted portfolio \( X(\mathbf{w}) = \sum_{j=1}^n w_i X_i, \mathbf{w} \in \mathcal{O} \) and the risk functional
\( f_\rho(\mathbf{w}) = \rho(X(\mathbf{w})) \)
Since \(f_{\rho}\) is positive homogeneous of order 1, Euler's rule implies
\[f_{\rho}(\mathbf{w}) = \sum_{i=1}^n \frac{\partial f_{\rho}}{\partial w_i}(\mathbf{w}) \cdot w_i, \quad \text{hence} \quad \rho(X) = f_{\rho}(\mathbf{1}) = \sum_{i=1}^n \frac{\partial f_{\rho}}{\partial w_i}(\mathbf{1}).\]

Let \(f_{\rho}\) be positive homogeneous and differentiable at \(\mathbf{w} = \mathbf{1}\). 
Then the allocation \[\rho_i(X|X) := \frac{\partial f_{\rho}}{\partial w_i}(\mathbf{1}), \quad i=1, \dots, n,\]
is called the Euler Capital Allocation Principle.

Remarks:
- The Euler principle satisfies property 1 (complete capital allocation).
- Property 2 (diversification) is satisfied if \(\rho\) is convex.


What is RoRaC-compatibility, and how is it related to the Euler allocation?
...
denote the RoRaC of the total portfolio \(X\) and of the sub-portfolio \(X_k\) respectively. A capital allocation \(\rho(X_k \vert X)\) is said to be 
RoRaC compatible if there exists an \(\varepsilon > 0\) such that
...
Under suitable differentiability assumptions for a coherent risk measure \(\rho\), the RoRaC compatible capital allocation \(\rho(X_k|X)\) (if exists) 
is unique and given by the Euler allocation:
\[\rho(X_k|X) = \frac{d}{dh} \rho(X + hX_k)|_{h=0}\]
