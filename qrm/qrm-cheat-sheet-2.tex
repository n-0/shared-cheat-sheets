...
with corresponding
\( \mathbf{C}^\top = (\delta_{i, j+1})_i = (0, \dots, 1, \dots, 0) \) (vector)
\( d = 0 \) (scalar)

This setup tries to detect if it's necessary to include the covariate \(x_j\)

Composite test:
\( H_0: \gamma_1 = 0 \quad \text{vs} \quad H_1: \gamma_1 \neq 0, \, \gamma_1 = (\beta_1, \beta_2, \ldots, \beta_r)^\top \)
...
0 & 0 & 0 & \cdots & 1 & 0 \\
\end{pmatrix} \] (matrix)
\( \mathbf{d} = \mathbf{0} \) (vector)
This setup tries to detect if it's necessary to include a set of covariates \( \{x_1, \dots, x_r \} \),
and is encountered typically for categorical/dummy encoding variables and is a generalization of the test of significance.

Test of equality:
\( H_0: \beta_j - \beta_r = 0 \quad \text{vs} \quad H_1: \beta_j - \beta_r \neq 0 \)

\( \mathbf{C}^\top = (\delta_{i, j+1} - \delta_{i, r+1})_i = (0, \dots, 1, 0, \dots, -1, \dots, 0) \) (vector)
\( d = 0 \) (scalar)
This setup tries to detect if the information of two variables conveyed, might be satisfactory given by just providing one of them.
